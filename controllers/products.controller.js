...
class ProductsController {
  ...
  getProductById = async (req, res, next) => {
    try {
      ...
      if (sessionUser) {
        ...
        const sessionCart = await cartController.getCartById(sessionUser.cart,next)
      
        return res.render('singleProduct.ejs',{theProduct, sessionCart});
      }else{
       
        return res.render('singleProduct.ejs', {theProduct});
      }
  ...
  saveNewProduct = async (req, res, next) => {
    try {
      ...
      if (!name || !desc || !image || !price || !stock ) 
